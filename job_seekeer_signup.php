...
<head>
    ...
    <title>Sign Up As Job Seeker</title>
    <style>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-offset-3 col-md-4">
              <h2 class="page-header">Sign Up As Job Seeker</h2>
               <br />
            <p style="color:red;font-weight:bold;" id="result"></p>
              <form action="" method="post" >
                   <div class="form-group">
                    <label for="User">UserName:</label>
                     <input type="text" class="form-control" name="j_username" id="User" required>
                     </div>

                     <div class="form-group">
                     <label for="em">Email:</label>
                      <input type="email" name="j_email" class="form-control"id="em" required>
                     </div>

                     <div class="form-group">
                     <label for="pas">Password:</label>
                      <input type="password" name="j_pass" class="form-control" id="pas" required>
                      </div>
                      
                      <div class="form-group">
                      <label for="cp">Confirm Password:</label>
                     <input type="password" name="j_confirmpass" class="form-control" id="cp" required>
                     </div>
                     <button type="submit" name="j_signup_submit" class="btn">Sign Up</button>
...
if(isset($_POST['j_signup_submit'])){
  echo $username=checker($_POST['j_username']);
    $email=checker($_POST['j_email']);
    $pass=checker($_POST['j_pass']);
   $confirmpass=checker($_POST['j_confirmpass']);
...
elseif($pass!==$confirmpass){
  ...
  $err='Password does not match';
   echo "<script>var error=document.getElementById('result');
	...
	$query1="select * from job_seeker where J_user_name='$username'";
	$runq_1=mysqli_query($conn,$query1);
	$check1=mysqli_num_rows($runq_1);
	$query2="select * from job_seeker where Job_seeker_email='$email'";
	...
	if($check1==0 AND $check2==0){
         //creating session for Job Seeker alright
     $_SESSION['Job_seeker_email']=$email;
$query3="insert into job_seeker(J_user_name,Job_seeker_email,job_seeker_password,testing) values('$username','$email','$encrypted_txt','$pass')";

		$runq_3=mysqli_query($conn,$query3);

		if($runq_3){
		echo "<script>window.open('Job_seeker/Job_seeker_profile.php','_self')</script>";
		}
	}
